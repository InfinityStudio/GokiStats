...
public class StatStealth extends StatLeaper
{
  ...
  public float[] getAppliedDescriptionVar(EntityPlayer player)
  {
    ...
    float newReap = DataHelper.trimDecimals(reap + reap * reapBonus / 100.0F, 1);
    return new float[]{speed,reapBonus,newReap};
  ...
  {
	  return StatCollector.translateToLocalFormatted(this.key+".des", this.getAppliedDescriptionVar(player)[0], this.getAppliedDescriptionVar(player)[1], this.getAppliedDescriptionVar(player)[2]);
  }
}