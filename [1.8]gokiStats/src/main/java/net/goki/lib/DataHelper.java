...
public class DataHelper
{
	...
	public static float getDamageDealt(EntityPlayer player, Entity target, DamageSource source)
	{
		...
		if (targetIsLiving)
		{
			bonusDamage = EnchantmentHelper.func_152377_a(	stack,
															((EntityLivingBase) target).getCreatureAttribute());
		}
		if ((damage > 0.0F) || (bonusDamage > 0.0F))
		{
			critical = (player.fallDistance > 0.0F) && (!player.onGround) && (!player.isOnLadder()) && (!player.isInWater()) && (!player.isPotionActive(Potion.blindness)) && (player.ridingEntity == null) && (targetIsLiving);
