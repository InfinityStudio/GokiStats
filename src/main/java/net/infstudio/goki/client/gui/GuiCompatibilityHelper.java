...
//        StatBase.stats.forEach(StatBase::reloadConfig);
        for (int i = 0; i < this.compatibleStats.length; i++) {
            if (this.compatibleStats[i].isEffectiveOn(this.player.getHeldItemMainhand())) {
                this.buttons.get(i).setMessage(ITextComponent.getTextComponentOrEmpty("Remove item from " + this.compatibleStats[i].getLocalizedName() + " list."));
                ((GuiExtendedButton) this.buttons.get(i)).setBackgroundColor(3381555);
            } else {
                this.buttons.get(i).setMessage(ITextComponent.getTextComponentOrEmpty("Add item to " + this.compatibleStats[i].getLocalizedName() + " list."));
